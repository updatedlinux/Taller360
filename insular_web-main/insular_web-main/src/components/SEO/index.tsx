...
const SEO = ({
  ...
  ogImageAlt = 'Insular Casa de Cambio',
  twitterCard = 'summary_large_image',
  noindex = false,
  nofollow = false,
  structuredData,
}: SEOProps) => {
  const siteUrl = 'https://insular.io';
  const fullUrl = canonical ? `${siteUrl}${canonical}` : siteUrl;
  const fullOgImage = ogImage.startsWith('http') ? ogImage : `${siteUrl}${ogImage}`;

  const fullTitle = title.includes('Insular') ? title : `${title} | Insular Casa de Cambio`;
  ...
  return (
    <Helmet>
      ...
      <meta property="og:site_name" content="Insular Casa de Cambio" />
